      ...
      with ops.control_dependencies(local_center_vars_update):
        ...
        for lvar, diff in zip(local_vars, differences):
          with ops.device(lvar.device): 
            flocking_local = tf.minimum((self._rep/self._dis)*tf.abs(diff)-self._rep+self._att, self._att)
            update_ops.append(
                state_ops.assign_sub(lvar, math_ops.multiply(flocking_local, diff)))
                                                       
                                     
        for var, diff in zip(global_center_vars, differences):
          with ops.device(var.device):
            flocking_g = tf.minimum((self._rep/self._dis)*tf.abs(diff)-self._rep+self._att, self._att)
            update_ops.append(
                state_ops.assign_add(var, math_ops.multiply(flocking_g, diff)/4))
